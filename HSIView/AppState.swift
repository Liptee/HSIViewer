...
private struct CubeMetricsPreparationContext {
    let entryID: CubeLibraryEntry.ID
    let displayName: String
...
private enum CubeMetricsComputationError: LocalizedError {
    case emptyData
...
private enum CubeMetricsEngine {
...
final class AppState: ObservableObject {
    ...
    @Published var cubeMetricsSelectionSourceID: CubeLibraryEntry.ID?
    @Published var cubeMetricsRequest: CubeMetricsRequest?
    ...
    var isCubeMetricsSelectionMode: Bool {
        cubeMetricsSelectionSourceID != nil
    }

    var cubeMetricsSelectionSourceEntry: CubeLibraryEntry? {
        guard let sourceID = cubeMetricsSelectionSourceID else { return nil }
        return libraryEntry(for: sourceID)
    }

    var cubeMetricsSelectionSourceDisplayName: String? {
        cubeMetricsSelectionSourceEntry?.displayName
    }
    ...
    func beginCubeMetricsSelection(from sourceID: CubeLibraryEntry.ID) {
        guard !isBusy else { return }
        guard libraryEntries.count > 1 else { return }
        guard libraryEntry(for: sourceID) != nil else { return }
        cubeMetricsRequest = nil
        cubeMetricsSelectionSourceID = sourceID
    }

    func cancelCubeMetricsSelection() {
        cubeMetricsSelectionSourceID = nil
    }

    func selectCubeMetricsTarget(_ targetID: CubeLibraryEntry.ID) {
        guard !isBusy else { return }
        guard let sourceID = cubeMetricsSelectionSourceID else { return }
        guard sourceID != targetID else { return }
        guard let sourceEntry = libraryEntry(for: sourceID),
              let targetEntry = libraryEntry(for: targetID) else {
            cubeMetricsSelectionSourceID = nil
            loadError = L("cube.metrics.error.entry_not_found")
            return
        }

        let sourceContext = metricsPreparationContext(for: sourceEntry)
        let targetContext = metricsPreparationContext(for: targetEntry)
        cubeMetricsSelectionSourceID = nil

        guard let sourceContext, let targetContext else {
            loadError = L("cube.metrics.error.prepare_unknown")
            return
        }

        beginBusy(message: L("cube.metrics.busy.prepare"))

        processingQueue.async { [weak self] in
            guard let self else { return }
            let sourcePrepared = self.prepareCubeForMetrics(using: sourceContext)
            let targetPrepared = self.prepareCubeForMetrics(using: targetContext)

            DispatchQueue.main.async {
                defer { self.endBusy() }

                guard let sourcePrepared else {
                    self.loadError = LF("cube.metrics.error.prepare_entry", sourceContext.displayName)
                    return
                }
                guard let targetPrepared else {
                    self.loadError = LF("cube.metrics.error.prepare_entry", targetContext.displayName)
                    return
                }

                guard sourcePrepared.signature == targetPrepared.signature else {
                    let a = sourcePrepared.signature
                    let b = targetPrepared.signature
                    self.loadError = LF(
                        "cube.metrics.error.incompatible",
                        a.width,
                        a.height,
                        a.channels,
                        b.width,
                ...
                self.loadError = nil
                self.cubeMetricsRequest = CubeMetricsRequest(reference: sourcePrepared, target: targetPrepared)
            }
        }
    }

    func calculateCubeMetrics(
        request: CubeMetricsRequest,
        settings: CubeMetricsSettings,
        completion: @escaping (Result<CubeMetricsResult, Error>) -> Void
    ) {
        beginBusy(message: L("cube.metrics.busy.calculate"))
        processingQueue.async {
            let result = Result {
                try CubeMetricsEngine.calculate(
    ...
    func removeLibraryEntry(_ entry: CubeLibraryEntry) {
        ...
        if cubeMetricsSelectionSourceID == entry.id {
            cubeMetricsSelectionSourceID = nil
        }
        if cubeMetricsRequest?.reference.entryID == entry.id || cubeMetricsRequest?.target.entryID == entry.id {
            cubeMetricsRequest = nil
        }
    }
    ...
    private func metricsPreparationContext(for entry: CubeLibraryEntry) -> CubeMetricsPreparationContext? {
        let canonical = canonicalURL(entry.url)
        let snapshot = snapshot(for: entry) ?? .empty
        let isCurrentEntry = cubeURL?.standardizedFileURL == canonical
        let currentRawCube: HyperCube? = isCurrentEntry ? (originalCube ?? cube) : nil
        if isCurrentEntry, currentRawCube == nil {
            return nil
        }

        return CubeMetricsPreparationContext(
            entryID: entry.id,
            displayName: entry.displayName,
            canonicalURL: canonical,
            snapshot: snapshot,
            currentRawCube: currentRawCube
        )
    }

    private func prepareCubeForMetrics(using context: CubeMetricsPreparationContext) -> CubeMetricsPreparedCube? {
        let rawCube: HyperCube
        if let currentRawCube = context.currentRawCube {
            rawCube = currentRawCube
        } else {
            _ = SecurityScopedBookmarkStore.shared.startAccessingIfPossible(url: context.canonicalURL)
            guard let loadedCube = tryLoadCube(for: context.canonicalURL) else {
                return nil
            }
            rawCube = loadedCube
        }

        guard let prepared = prepareCubeForExport(cube: rawCube, snapshot: context.snapshot),
              let signature = cubeMetricsSignature(cube: prepared.cube, layout: prepared.layout) else {
            return nil
        }

        return CubeMetricsPreparedCube(
    ...
    private func cubeMetricsSignature(cube: HyperCube, layout: CubeLayout) -> CubeMetricsSpatialSignature? {
        ...
        return CubeMetricsSpatialSignature(width: width, height: height, channels: channels)
    }
