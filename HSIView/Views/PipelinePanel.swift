...
struct OperationEditorView: View {
    ...
    @State private var showAutoWhitePointSearch: Bool = false
    ...
    private func loadLocalState() {
        ...
        case .calibration:
            ...
            showAutoWhitePointSearch = false
        case .resize:
    ...
    private func calibrationEditor(for op: PipelineOperation) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Toggle(state.localized("Автоматический поиск области белого"), isOn: $localCalibrationParams.autoWhiteSearchEnabled)
                    .font(.system(size: 10))
                    .toggleStyle(.checkbox)
                    .controlSize(.small)

                if localCalibrationParams.autoWhiteSearchEnabled {
                    VStack(alignment: .leading, spacing: 6) {
                        Button {
                            showAutoWhitePointSearch = true
                        } label: {
                            HStack(spacing: 6) {
                                Image(systemName: "sparkles.rectangle.stack")
                                Text(state.localized("Открыть анализ сцены"))
                            }
                            .font(.system(size: 10, weight: .medium))
                        }
                        .buttonStyle(.bordered)
                        ...
                        Text(state.localized("Откроется окно RGB-синтеза, найдутся кандидаты белой области с учётом засветок, однородности и контраста."))
                            .font(.system(size: 9))
                            .foregroundColor(.secondary)
                    }
                    .padding(8)
            ...
            .sheet(isPresented: $showAutoWhitePointSearch) {
                autoWhitePointSheet(for: op)
            }
    ...
    @ViewBuilder
    private func autoWhitePointSheet(for op: PipelineOperation) -> some View {
        if let cube = state.cube {
            let layout = resolvedLayoutForAutoCrop(operation: op, cube: cube)
            AutoWhitePointSearchWindow(
                cube: cube,
                layout: layout,
                preset: localCalibrationParams.autoWhiteSearchPreset,
                windowPreset: localCalibrationParams.autoWhiteWindowPreset,
                previewImage: autoWhitePreviewImage(cube: cube, layout: layout),
                wavelengths: resolvedWavelengthsForAutoWhite(cube: cube, layout: layout),
                onPresetChange: { newPreset in
                    localCalibrationParams.autoWhiteSearchPreset = newPreset
                },
                onWindowPresetChange: { newPreset in
                    localCalibrationParams.autoWhiteWindowPreset = newPreset
                }
            ) { candidate, rank in
                applyAutoWhiteCandidate(candidate, rank: rank)
                showAutoWhitePointSearch = false
            }
        } else {
            VStack(spacing: 12) {
                Image(systemName: "exclamationmark.triangle")
                    .font(.system(size: 20))
                    .foregroundColor(.orange)
                Text(state.localized("Откройте ГСИ перед автопоиском белой области"))
                    .font(.system(size: 12))
                    .foregroundColor(.secondary)
                Button(state.localized("Закрыть")) {
                    showAutoWhitePointSearch = false
                }
                .buttonStyle(.borderedProminent)
            }
    ...
    private func resolvedWavelengthsForAutoWhite(cube: HyperCube, layout: CubeLayout) -> [Double]? {
        let channels = cube.channelCount(for: layout)
        if let wl = cube.wavelengths, wl.count == channels {
            return wl
        }
        if let wl = state.wavelengths, wl.count == channels {
            return wl
        }
        return nil
    }

    private func autoWhitePreviewImage(cube: HyperCube, layout: CubeLayout) -> NSImage? {
        let channels = cube.channelCount(for: layout)
        guard channels > 0 else { return nil }
        let previewSize = CGSize(width: 1280, height: 1280)
        if channels == 1 {
            return ImageRenderer.renderGrayscale(
                cube: cube,
                layout: layout,
                channelIndex: 0,
                targetPixels: previewSize
            )
        }

        let wavelengths = resolvedWavelengthsForAutoWhite(cube: cube, layout: layout)
        let current = state.colorSynthesisConfig.mapping.clamped(maxChannelCount: channels)
        let mapping: RGBChannelMapping
        if current.red == current.green && current.green == current.blue {
            mapping = RGBChannelMapping.defaultMapping(channelCount: channels, wavelengths: wavelengths)
        } else {
            mapping = current
        }
    ...
    private func applyAutoWhiteCandidate(_ candidate: WhitePointCandidate, rank: Int) {
        let rect = candidate.rect
        let sourceName = LF(
            "pipeline.calibration.auto_white.candidate_name",
            rank,
            rect.minX,
            rect.minY,
            rect.width,
        ...
        localCalibrationParams.whiteSpectrum = CalibrationSpectrum(
            values: candidate.meanSpectrum,
...
private struct AutoWhitePointSearchWindow: View {
    let cube: HyperCube
...
private struct WhitePointCandidatePreview: View {
    let image: NSImage?
    let pixelWidth: Int
    let pixelHeight: Int
...
private struct WhitePointCandidateMetricsView: View {
...
private struct WhitePointSpectrumChart: View {
    let values: [Double]
    let wavelengths: [Double]?
    let height: CGFloat
