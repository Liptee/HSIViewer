...
- **Система пайплайнов обработки**: последовательное применение операций в произвольном порядке
  ...
  - Автоматическое применение при любых изменениях
...
- **Панель нормализации**: 7 типов нормализаций с настраиваемыми параметрами
  - Без нормализации (оригинальные данные)
  - Min-Max (0-1) - линейная нормализация
  - Min-Max (custom) - с настраиваемым диапазоном
  - Percentile - обрезка выбросов (2%-98% по умолчанию)
  - Z-Score - стандартизация для ML
  - Log - логарифмическое преобразование
  - Sqrt - квадратный корень
  - Динамические поля для параметров
  - Возможность вернуться к оригиналу
  ...
  - **Автоматическое обновление информационной панели** при применении преобразований
    ...
    - Работает для любых будущих преобразований (фильтры, PCA, и т.д.)
    - Использует SwiftUI `.onChange()` и `.id()` для реактивности

- **Панель конвертации типов данных**: изменение типа данных гиперкуба
  - Поддержка 7 типов: Float64/32, UInt8/16, Int8/16/32
  - Два режима конвертации:
    - Автоматическое масштабирование (сохраняет относительные значения)
    - Обрезка значений (clamping)
  - Показ изменения размера в памяти (↓ 8.0x или ↑ 2.0x)
  - Оценка размера до применения конвертации
  - Автоматическое обновление информационной панели
  - Примеры использования:
    - Float64 → UInt8: экономия памяти в 8 раз
    - UInt8 → Float32: подготовка для машинного обучения
    - UInt16 → UInt8: масштабирование с сохранением деталей
