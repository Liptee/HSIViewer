...
- **Критическое исправление NPY Fortran-order**: неправильное отображение (вертикальные полосы)
  - Добавлено переупорядочивание данных из Fortran-order в C-order
  - Учитывается флаг `fortranOrder` из заголовка NPY
  - Правильная индексация: `h + H*(w + W*c)` → `c + C*(w + W*h)`
  - ⚠️ Временно удваивает использование памяти при загрузке
